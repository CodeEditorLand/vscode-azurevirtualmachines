...
suite("Create virtual machine", function (this: Mocha.Suite) {
	...
	test("Basic Create", async () => {
		...
		const testInputs: (string | RegExp)[] = [resourceName, "", location];
	...
	test("Advanced Create - Linux", async () => {
		const linuxPasswordInputs: IPasswordInput[] = [
			standardPasswordInput,
			{
				title: "no password",
				input: [""],
		...
		const parallelTests: Promise<void>[] = await createVmTestsByOs(
			"Linux",
			linuxPasswordInputs
		);
	...
	test("Advanced Create - Windows", async () => {
		...
		const parallelTests: Promise<void>[] = await createVmTestsByOs(
			"Windows",
			windowsPasswordInputs
		);
		await Promise.all(parallelTests);
	});
});

async function testCreateVirtualMachine(
	os: string,
	image: string,
...
async function createVmTestsByOs(
	os: OperatingSystemType,
	passwordInputs: IPasswordInput[]
...
async function verifyVmCreated(
