...
import {
	ResolvedVirtualMachineTreeItem,
...
export async function createSshKey(
	...
	return await callWithMaskHandling(async () => {
		...
		if (!keyExists) {
			...
			if (await sshKeygenExists()) {
				...
				await cpUtils.executeCommand(
					...
					"4096",
					"-f",
					cpUtils.wrapArgInQuotes(sshKeyPath),
					"-N",
				...
				if (passphrase) {
					...
					ext.outputChannel.appendLog(
						localize(
							"unableToFindKeygen",
							"NOTE: Unable to find `ssh-keygen` in environment variable PATH.  Passphrase will not be respected.",
							vmName
						)
					);
				}
		...
		return {
			sshKeyName,
			keyData: (await fse.readFile(`${sshKeyPath}.pub`)).toString(),
		};
	}, passphrase);
}

export async function configureSshConfig(
	context: IActionContext,
	vmti: ResolvedVirtualMachineTreeItem | VirtualMachineTreeItem,
	sshKeyPath: string
): Promise<void> {
	...
	if (hostEntry.Host) {
		...
		while (
			!!sshConfig.compute(`${vmti.name}-${count}`).Host &&
			count < 100
		) {
			count = count + 1;
		}
	...
	ext.outputChannel.appendLog(
		localize(
			"addingEntry",
			`Added new entry to "{0}" with Host "{1}".`,
			sshConfigPath,
			host
		)
	);
