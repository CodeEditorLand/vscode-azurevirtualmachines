...
export class PassphrasePromptStep extends AzureWizardPromptStep<IVirtualMachineWizardContext> {
	...
	): Promise<IWizardOptions<IVirtualMachineWizardContext> | undefined> {
	...
	private validatePassphrase(
		...
		return !isWindows
			? this.validateLinuxPassphrase(value)
			: this.validateWindowsPassword(value);
	}

	private validateLinuxPassphrase(
		value: string | undefined,
	): string | undefined {
		...
		if (value && value.length < passphraseMinLength) {
			return localize(
				"invalidLength",
				"The passphrase must be at least {0} characters or empty for no passphrase.",
				passphraseMinLength,
			);
		} else {
			return undefined;
	...
	private validateWindowsPassword(
		value: string | undefined,
	): string | undefined {
		...
		if (!value) {
			...
			return localize(
				"invalidLength",
				"The password must be between {0} and {1} characters long",
				passwordMinLength,
				passwordMaxLength,
			);
