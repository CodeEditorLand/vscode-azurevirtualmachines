...
export class NetworkInterfaceCreateStep extends AzureWizardExecuteStep<IVirtualMachineWizardContext> {
	...
	public async execute(
		...
		const newLocation: AzExtLocation = await LocationListStep.getLocation(
			context,
			undefined,
			true,
		);
		...
		context.newNetworkInterfaceName =
			context.newNetworkInterfaceName ||
			this.formatNetworkInterfaceName(vmName);

		const publicIpAddress: PublicIPAddress = nonNullProp(
			context,
			"publicIpAddress",
		);
		...
		const networkInterfaceProps: NetworkInterface = {
			...
			ipConfigurations: [
				{
					name: context.newNetworkInterfaceName,
		...
		const creatingNi: string = localize(
			"creatingNi",
		...
		context.networkInterface = await networkClient.networkInterfaces.get(
			rgName,
			context.newNetworkInterfaceName,
		);
		ext.outputChannel.appendLog(
			localize(
				"createdNi",
				`Created new network interface "${context.newNetworkInterfaceName}".`,
			),
		);
	}
