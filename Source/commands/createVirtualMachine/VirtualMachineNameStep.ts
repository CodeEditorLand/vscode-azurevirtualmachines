...
export class VirtualMachineNameStep extends AzureNameStep<IVirtualMachineWizardContext> {
	...
	private async validateVirtualMachineName(
		context: IVirtualMachineWizardContext,
		...
		if (
			name.length < virtualMachineNamingRules.minLength ||
			name.length > virtualMachineNamingRules.maxLength
		) {
			return localize(
				"invalidLength",
				"The name must be between {0} and {1} characters.",
				virtualMachineNamingRules.minLength,
				virtualMachineNamingRules.maxLength,
			);
		} else if (virtualMachineNamingRules.invalidCharsRegExp.test(name)) {
			return localize(
				"invalidChars",
				"The name can only contain alphanumeric characters and the symbols '.' and '-'",
			);
		} else if (
			name.startsWith(".") ||
			name.endsWith(".") ||
			...
			return localize(
				"invalidEndingChar",
				"The name cannot start or end in a '.' or '-'.",
			);
		} else if (
			context.resourceGroup?.name &&
			!(await this.isNameAvailableInRG(
				context,
				context.resourceGroup.name,
				name,
			))
			...
			return localize(
				"nameAlreadyExists",
				'Virtual machine name "{0}" already exists in resource group "{1}".',
				name,
				context.resourceGroup.name,
			);
		} else {
	...
	private async isNameAvailableInRG(
		context: IVirtualMachineWizardContext,
		rgName: string,
		name: string,
	): Promise<boolean> {
