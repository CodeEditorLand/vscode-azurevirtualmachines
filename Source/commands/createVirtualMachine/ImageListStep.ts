...
export class ImageListStep extends AzureWizardPromptStep<IVirtualMachineWizardContext> {
	public async prompt(context: IVirtualMachineWizardContext): Promise<void> {
		...
		if ("legacyPlanId" in image.data) {
			const plan = await this.getPlanFromLegacyPlanId(
				context,
				image.data,
			);
	...
	public async getDefaultImageReference(
		...
		const defaultImage =
			images.find((i) =>
				/UbuntuServer1804LTS18_04/.test(i.legacyPlanId),
			) || images[0];
	...
	public async getQuickPicks(
		context: IActionContext,
		os?: OperatingSystemType,
	): Promise<IAzureQuickPickItem<FeaturedImage | ImageReference>[]> {
		const featuredImages: IAzureQuickPickItem<
	...
	private async getFeaturedImages(
		context: IActionContext,
		...
		const options: AzExtRequestPrepareOptions = {
			...
			url: createRequestUrl(
				"https://catalogapi.azure.com/catalog/curationgrouplisting",
				{
					...
					"returnedProperties":
						"operatingSystem.family,id,image,freeTierEligible,legacyPlanId",
				},
			),
		};
	...
	private async getPlanFromLegacyPlanId(
		context: IActionContext,
		featuredImage: FeaturedImage,
	): Promise<PlanFromLegacyPlanId> {
		const getOfferOptions: AzExtRequestPrepareOptions = {
			method: "GET",
			url: createRequestUrl(
				`https://euap.catalogapi.azure.com/view/offers/${featuredImage.legacyPlanId}`,
				apiVersionQueryParam,
			),
		};

		const offer = <OfferFromLegacyPlanId>(
			(
				await sendRequestWithTimeout(
					context,
					getOfferOptions,
		...
		if (!plan) {
			throw new Error(
				localize("noPlan", "Could not find plan from featured offer."),
			);
		}
	...
	private async getImageReference(
		context: IActionContext,
		plan: PlanFromLegacyPlanId,
	): Promise<ImageReference> {
		...
		if (!uiDefUri) {
			throw new Error(
				localize(
					"noUiDefUri",
					"Could not find image reference from featured offer.",
				),
			);
		}
		...
		const createdUiDefintion = <UiDefinition>(
			(
				await sendRequestWithTimeout(
					context,
					getUiDefOptions,
	...
	private getExtraImageQuickPicks(
		os?: OperatingSystemType,
	): IAzureQuickPickItem<ImageReference>[] {
		if (os === "Windows") {
			return [];
		} else {
			// defaults to Linux
