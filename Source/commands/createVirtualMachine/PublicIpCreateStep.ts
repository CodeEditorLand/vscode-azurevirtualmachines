...
export class PublicIpCreateStep extends AzureWizardExecuteStep<IVirtualMachineWizardContext> {
	...
	public async execute(
		...
		const ipName: string =
			nonNullProp(context, "newVirtualMachineName") + "-ip";

		const creatingIp: string = localize(
			"creatingIp",
		...
		try {
			...
			context.publicIpAddress = await networkClient.publicIPAddresses.get(
				rgName,
				ipName,
			);
		} catch (e) {
			if (extendedLocation) {
				throw new Error(
					localize(
						"mayNotBeSupportedInEdgeZones",
						"Failed to create Virtual Machine. This image may not be supported in Edge Zones.",
					),
				);
			}
			throw e;
		...
		ext.outputChannel.appendLog(
			localize(
				"creatingIp",
				`Created new public IP addresss "${ipName}".`,
			),
		);
	}
