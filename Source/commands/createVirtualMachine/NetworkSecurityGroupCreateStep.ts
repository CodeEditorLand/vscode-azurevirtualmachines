...
export class NetworkSecurityGroupCreateStep extends AzureWizardExecuteStep<IVirtualMachineWizardContext> {
	...
	public async execute(
		...
		const nsgName: string =
			nonNullProp(context, "newVirtualMachineName") + "-nsg";
		...
		const enabledPorts: string[] = nonNullProp(
			networkSecurityGroupProps,
			"securityRules"
		).map((rule) => nonNullProp(rule, "destinationPortRange"));

		const creatingNsg: string = localize(
			"creatingNsg",
			`Creating new network security group "${nsgName}" with inbound ports ${enabledPorts.join(
				", "
			)} enabled...`
		);
		...
		ext.outputChannel.appendLog(
			localize(
				"createdNsg",
				`Created new network security group "${nsgName}".`
			)
		);
	}
