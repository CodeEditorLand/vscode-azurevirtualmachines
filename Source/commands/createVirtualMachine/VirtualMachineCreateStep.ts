	...
	public async execute(
		...
		const storageProfile: StorageProfile = {
			...
			osDisk: {
				...
				managedDisk: {
					storageAccountType: context.isCustomCloud
						? "Standard_LRS"
						: "Premium_LRS",
				},
			},
		...
		const networkInterface: NetworkInterface = nonNullProp(
			context,
			"networkInterface"
		);
		...
		const virtualMachineProps: VirtualMachine = {
			location,
			extendedLocation,
			hardwareProfile,
			storageProfile,
		...
		const creatingVm: string = localize(
			"creatingVm",
		...
		const creatingVmDetails: string = localize(
			...
			`${nonNullProp(storageProfile, "imageReference").offer} ${
				nonNullProp(storageProfile, "imageReference").sku
			}`
		);

		const createdVm: string = localize(
			"creatingVm",
			'Created new virtual machine "{0}".',
			vmName
		);
		...
		void window
			.showInformationMessage(createdVm, viewOutput)
