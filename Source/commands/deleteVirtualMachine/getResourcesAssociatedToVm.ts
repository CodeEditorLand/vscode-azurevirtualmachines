...
export async function getResourcesAssociatedToVm(
	...
	if (node.data.networkProfile?.networkInterfaces) {
		for (const networkRef of node.data.networkProfile.networkInterfaces) {
			if (networkRef.id) {
				networkReferences.push({
					name: getNameFromId(networkRef.id),
					rgName: getResourceGroupFromId(networkRef.id),
				});
			}
		}
	}

	const networkClient: NetworkManagementClient = await createNetworkClient([
		context,
		node?.subscription,
	]);
	...
	if (node.data.storageProfile?.osDisk?.managedDisk?.id) {
		...
		associatedResources.push({
			...
			deleteMethod: async (): Promise<void> => {
				await computeClient.disks.beginDeleteAndWait(diskRg, diskName);
			},
		});
	}
