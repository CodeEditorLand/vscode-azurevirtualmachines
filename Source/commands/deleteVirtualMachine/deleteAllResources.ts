...
export async function deleteAllResources(
	...
	await Promise.all(
		parallelResources.map(
			async (r) => await deleteWithOutput(r, failedResources)
		)
	);

	const resourceClient: ResourceManagementClient = await createResourceClient(
		...
		await deleteWithOutput(
			{
				...
				resourceType: localize("resourceGroup", "resource group"),
