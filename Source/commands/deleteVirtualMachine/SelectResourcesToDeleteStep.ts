...
export class SelectResourcesToDeleteStep extends AzureWizardPromptStep<IDeleteChildImplContext> {
	public async prompt(context: IDeleteChildImplContext): Promise<void> {
		const resourcesToDelete: IAzureQuickPickItem<ResourceToDelete>[] =
			await context.ui.showQuickPick(
				getQuickPicks(context, nonNullProp(context, "node")),
				{
...
async function getQuickPicks(
...
): Promise<IAzureQuickPickItem<ResourceToDelete>[]> {
	const resources: ResourceToDelete[] = await getResourcesAssociatedToVm(
		context,
		node,
	);
	...
	resources.unshift({
		...
		resourceType: virtualMachineLabel,
		picked: true,
