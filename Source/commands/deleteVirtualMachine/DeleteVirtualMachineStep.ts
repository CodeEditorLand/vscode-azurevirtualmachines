...
export class DeleteVirtualMachineStep extends AzureWizardExecuteStep<IDeleteChildImplContext> {
	...
	public async execute(
		...
		const failedResources: ResourceToDelete[] = await deleteAllResources(
			context,
			node.subscription,
			node.resourceGroup,
			resourcesToDelete,
		...
		const messageDeleteWithErrors: string = localize(
			...
			"Failed to delete the following resource(s): {0}.",
			failedResourceList,
		);

		const deleteSucceeded: string = multiDelete
			? localize(
					"DeleteSucceeded",
		...
		if (failedResources.length > 0) {
			context.telemetry.properties.failedResources =
				failedResources.length.toString();
			...
			if (
				...
				const viewOutputAzureButton: AzExtErrorButton = {
					title: viewOutput.title,
			...
			void context.ui.showWarningMessage(
				`${messageDeleteWithErrors} Check the [output channel](command:${ext.prefix}.showOutputChannel) for more information.`,
			);
		}
	}
