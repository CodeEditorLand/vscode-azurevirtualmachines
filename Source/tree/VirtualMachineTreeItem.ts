...
import {
	...
	type ResolvedAppResourceTreeItem,
...
import {
	type IDeleteChildImplContext,
	type ResourceToDelete,
} from "../commands/deleteVirtualMachine/deleteConstants";
...
export type ResolvedVirtualMachineTreeItem =
	ResolvedAppResourceTreeItem<ResolvedVirtualMachine> & AzExtTreeItem;
...
export class VirtualMachineTreeItem implements ResolvedVirtualMachine {
	...
	public getUser(): string {
		return nonNullValueAndProp(
			this.virtualMachine.osProfile,
			"adminUsername",
		);
	}
	...
	public async getIpAddress(context: IActionContext): Promise<string> {
		...
		if (networkInterfaces.length === 0) {
			throw new Error(
				localize(
					"noNicConfigs",
					'No network interfaces are associated with "{0}"',
					this.name,
				),
			);
		}
		...
		if (
			...
			throw new Error(
				localize(
					"noIpConfigs",
					'No IP configurations are associated with network interface "{0}"',
					networkInterface.name,
				),
			);
		}

		const publicIPAddressName: string = getNameFromId(
			nonNullValueAndProp(
				networkInterface.ipConfigurations[0].publicIPAddress,
				"id",
			),
		);
	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		...
		const wizard = new AzureWizard<IDeleteChildImplContext>(wizardContext, {
			promptSteps: [
				new SelectResourcesToDeleteStep(),
				new ConfirmDeleteStep(),
			],
		...
		const resourcesToDelete: ResourceToDelete[] = nonNullProp(
			wizardContext,
			"resourcesToDelete",
		);
		...
		wizardContext.activityTitle = multiDelete
			? localize("delete", "Delete {0}...", wizardContext.resourceList)
			: localize(
					"delete",
					'Delete {0} "{1}"...',
					resourcesToDelete[0].resourceType,
					resourcesToDelete[0].resourceName,
				);
	...
	private getStateFromInstanceView(
		instanceView: VirtualMachineInstanceView,
	): string | undefined {
		const powerState: InstanceViewStatus | undefined =
			instanceView.statuses &&
			instanceView.statuses.find((status): boolean =>
				status.code && status.code.toLowerCase().includes("powerstate")
					? true
					: false,
			);
